...
const serviceImpl = {
    echo: (call, callback) => {
        ...
        if (succeedOnRetryAttempt.length === 0 ||
            ...
            const code = statusCode[0]
                ? Number.parseInt(statusCode[0])
                : grpc.status.UNKNOWN;
            callback({
                code: code,
                details: `Failed on retry ${(_a = previousAttempts[0]) !== null && _a !== void 0 ? _a : 0}`,
            });
        }
    },
};
describe('Retries', () => {
    ...
    describe('Client with retries disabled', () => {
        ...
        it('Should fail if the server fails the first request', done => {
            ...
            client.echo({ value: 'test value', value2: 3 }, metadata, (error, response) => {
                assert(error);
    ...
    describe('Client with retries configured', () => {
        ...
        it('Should not be able to make more than 5 attempts', done => {
            ...
            metadata.set('respond-with-status', `${grpc.status.RESOURCE_EXHAUSTED}`);
            client2.echo({ value: 'test value', value2: 3 }, metadata, (error, response) => {
                assert(error);
