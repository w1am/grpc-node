...
const channelzGrpcObject = grpc.loadPackageDefinition(loadedChannelzProto);
const TestServiceClient = (0, common_1.loadProtoFile)(`${__dirname}/fixtures/test_service.proto`).TestService;
...
describe('Channelz', () => {
                        ...
                        channelzClient.getServer({ server_id: testServer.getChannelzRef().id }, (error, serverResult) => {
                            ...
                            channelzClient.getServerSockets({ server_id: testServer.getChannelzRef().id }, (error, socketsResult) => {
                                ...
                                channelzClient.getSocket({
                                    ...
                                    assert.strictEqual(+serverSocketResult.socket.data
                                        .streams_succeeded, 1);
    ...
    it('should count failed calls', done => {
        testClient.unary({ error: true }, (error, value) => {
            assert(error);
            // Channel data tests
            channelzClient.GetChannel({ channel_id: testClient.getChannel().getChannelzRef().id }, (error, channelResult) => {
                assert.ifError(error);
...
describe('Disabling channelz', () => {
    ...
    it('Should still work', done => {
        ...
        testClient.unary({}, { deadline }, (error, value) => {
            assert.ifError(error);
            done();
        });
    });
});
//# sourceMappingURL=test-channelz.js.map