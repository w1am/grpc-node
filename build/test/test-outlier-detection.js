...
const load_balancer_outlier_detection_1 = require("../src/load-balancer-outlier-detection");
...
describe('Outlier detection config validation', () => {
    describe('interval', () => {
        it('Should reject a negative interval', () => {
            ...
            assert.throws(() => {
                load_balancer_outlier_detection_1.OutlierDetectionLoadBalancingConfig.createFromJson(loadBalancingConfig);
...
describe('Outlier detection', () => {
    ...
    function makeManyRequests(makeOneRequest, total, callback) {
        if (total === 0) {
            callback();
            return;
        }
    ...
    describe('Success rate', () => {
        let makeCheckedRequest;
        let makeUncheckedRequest;
        before(() => {
            const target = 'ipv4:///' +
            ...
            makeUncheckedRequest = (callback) => {
                client.echo({ value: 'test value', value2: 3 }, (error, response) => {
                    callback();
                });
            };
            makeCheckedRequest = (callback) => {
                client.echo({ value: 'test value', value2: 3 }, (error, response) => {
                    callback(error);
                });
            };
...
//# sourceMappingURL=test-outlier-detection.js.map